<?php
	...
	if (!function_exists('wurfl_text_field_subsets')) {
		function wurfl_text_field_subsets() {
			return array('code', 'length', 'size', 'no_of', 'byte', 'width', 'height', 'rate', 'column', 'row', 'user_agent', 'fallback', 'manufacture',
						'model', 'code', 'type', 'ua', 'string', 'charset', 'color', 'file_upload');
		}
	}
	...
	if (!function_exists("wurfl_adminMenu")) {
	  ...
	  function wurfl_footer_adminMenu()
	  {
			echo "</div></td></tr>";
	  		echo "</table>";
			echo "<div align=\"center\"><a href=\"http://www.xoops.org\" target=\"_blank\"><img src=" . ICMS_URL . '/' . $GLOBALS['wurflModule']->getInfo('icons32') . '/xoopsmicrobutton.gif'.' '." alt='ICMS' title='ICMS'></a></div>";
			echo "<div class='center smallsmall italic pad5'><strong>" . $GLOBALS['wurflModule']->getVar("name") . "</strong> is maintained by the <a class='tooltip' rel='external' href='http://www.xoops.org/' title='Visit ICMS Community'>ICMS Community</a> and <a class='tooltip' rel='external' href='http://www.chronolabs.coop/' title='Visit Chronolabs Co-op'>Chronolabs Co-op</a></div>";
	...
	if (!function_exists('wurfl_getFilterElement')) {
		function wurfl_getFilterElement($filter, $field, $sort='did', $op = '', $fct = '') {
			...
			include_once(dirname(__FILE__).'/form.objects.php');
			switch ($field) {
				case 'id':
				case 'user_agent':
				case 'fallback':
				...
				case 'series':
					...
					$ele->addElement(new icms_form_elements_Text('', 'filter_'.$field.'', 11, 40, $components['value']));
	...
	if (!function_exists('wurfl_getFilterURLComponents')) {
		function wurfl_getFilterURLComponents($filter, $field, $sort='did') {
			...
			return array('value'=>(isset($ele_value)?$ele_value:''), 'field'=>(isset($field)?$field:''), 'operator'=>(isset($operator)?$operator:''), 'filter'=>(count($ret)?implode('|', $ret):''), 'extra'=>(count($retb)?implode('&', $retb):''));
		}
	}
?>