<?php
	...
	case "agents":	
		...
		asort($ret, SORT_DESC);
		$ttl = count($ret);
		$pagenav = new icms_view_PageNav($ttl, $limit, $start, 'start', 'limit='.$limit.'&sort='.$sort.'&order='.$order.'&op='.$op.'&fct='.$fct.'&filter='.$filter.'&fct='.$fct.'&filter='.$filter);
		...
		$GLOBALS['xoopsTpl']->display('db:admin/wurfl_cpanel_list_useragents.html');
		break;		
