		...
		case "devices":	
			...
			switch ($fct)
			{
				...
				case "list":				
					...
					$criteria = $devices_handler->getFiltericms_db_criteria_Item($filter);
					...
					foreach($devicess as $did => $devices) {
						$GLOBALS['xoopsTpl']->append('devices', $devices->getValues());
					}
					$GLOBALS['xoopsTpl']->assign('form', wurfl_devices_form(false));
					$GLOBALS['xoopsTpl']->assign('php_self', $_SERVER['PHP_SELF']);
					$GLOBALS['xoopsTpl']->display('db:admin/wurfl_cpanel_devices_list.html');
					break;		
				...
				case "edit":
					...
					$GLOBALS['xoopsTpl']->assign('php_self', $_SERVER['PHP_SELF']);
					$GLOBALS['xoopsTpl']->display('db:admin/wurfl_cpanel_devices_edit.html');
					break;
				case "save":
				...
				case "delete":	
					...
					if (isset($_POST['did'])&&$id=intval($_POST['did'])) {
						...
						$devices = $devices_handler->get(intval($_REQUEST['did']));
						icms_confirm(array('did'=>$_REQUEST['did'], 'op'=>$_REQUEST['op'], 'fct'=>$_REQUEST['fct'], 'limit'=>$_REQUEST['limit'], 'start'=>$_REQUEST['start'], 'order'=>$_REQUEST['order'], 'sort'=>$_REQUEST['sort'], 'filter'=>$_REQUEST['filter']), 'devices.php', sprintf(_WURFL_AM_MSG_DEVICES_DELETE, $devices->getTitle()));
					}
					break;
			}
			break;
