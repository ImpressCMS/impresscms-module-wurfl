<?php
	...
	if (!defined('ICMS_ROOT_PATH')) die(_NOPERM);
	...
	$handler = icms_getModuleHandler($GLOBALS['op'], 'wurfl');
	switch($GLOBALS['fct']) {
		case 'list':
			...
			$GLOBALS['xoopsTpl']->display('db:admin/wurfl_cpanel_list_'.$GLOBALS['op'].'.html');
		...
		case 'create':
			...
			$GLOBALS['xoopsTpl']->display('db:admin/wurfl_cpanel_create_'.$GLOBALS['op'].'.html');
			echo wurfl_footer_adminMenu(false);
			icms_cp_footer();
			exit;
			break;
